...
function ToDo({ todo, toggleTask, removeTask, updateTask }) {
    ...
    useEffect(() => {
        ...
        const savedNoteText = localStorage.getItem('selectedNoteText');

        if (savedNoteId === todo.id) {
            setIsEditing(true);
            setNewText(savedNoteText);
        }
    }, [todo.id]);
    ...
    const handleEdit = () => {
        ...
        localStorage.removeItem('selectedNoteText');
    };
    ...
    const handleEditToggle = () => {
        ...
        if (!isEditing) {
            localStorage.setItem('selectedNoteId', todo.id);
            localStorage.setItem('selectedNoteText', todo.task);
        } else {
            localStorage.removeItem('selectedNoteId');
            localStorage.removeItem('selectedNoteText');
        }
    ...
    return (
        <div className={`item-todo ${todo.complete ? 'completed' : ''}`}>
            ...
            <div className="todo-actions">
                <button className='item-task' onClick={handleEditToggle}>
